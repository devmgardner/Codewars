            ...
            for i,x in enumerate(r):
                for validnum in validnums:
                    if not r.count(x) == 1 and not validnum in r:
                        fhand.write(f'row check, found error with number {x}\n')
                        r[i] = validnum
    cg(puzzle)
    cc(puzzle)
    cr(puzzle)
